...
(
    ...
    areas_Id [INT] NOT NULL DEFAULT 1,
    CONSTRAINT pkTecnico PRIMARY KEY(id)
    -- specify more columns here
);
GO

-- Create a new table called 'estados' in schema 'dbo'
-- Drop the table if it already exists
IF OBJECT_ID('dbo.estados', 'U') IS NOT NULL
...
(
    ...
    CONSTRAINT fkEstadoCasoTicket FOREIGN KEY(estado_Id) REFERENCES dbo.estados(id)
    -- specify more columns here
);
GO
...
( -- columns tcodigo_t data into
 [codigo_usuario], [nombre], [areas_id]
)
VALUES
( -- first row: values for the columns in the list above
 2211, 'Juan Pablo', 1
...
-- add more rows here
(
 3251, 'Maria del Carmen', 3
),
(
 3150, 'Pedro Antunez', 4
),
(
 3111, 'Linda Flores', 3
)
...
-- Insert rows into table 'estados'
INSERT INTO estados
( -- columns to insert data into
 [descripcion]
)
...
( -- columns to insert data into
 [problema],[estado_Id],[tecnicoId]
)
VALUES
( -- first row: values for the columns in the list above
 'Monitor no funciona porque no me muestra ninguna luz e imagen del computador',1,1
),
( -- second row: values for the columns in the list above
'El equipo tiene dos pitidos al arrancar y no pasa de pantalla del BIOS',2,2
),
-- add more rows here
( 
'No funciona los medios para la videollamada',3,1
),
( 
 'Equipo bloqueado despues de tres intentos de Login',2,1
)
GO
...
--WHERE 	/* add search conditions here */
GO
-- Select rows from a Table or View 'procedimientos' in schema 'dbo'
...
--WHERE 	/* add search conditions here */
GO
-- Select rows from a Table or View 'estados' in schema 'dbo'
SELECT * FROM dbo.estados
--WHERE 	/* add search conditions here */
GO
-- Select rows from a Table or View 'areas' in schema 'dbo'
